...
def add_subcategory():
    name = request.form.get("sub_name"), pid = request.form.get("parent_id")
    if name and pid:
        db.session.add(SubCategory(name=name, category_id=pid)); db.session.commit()
    return redirect(url_for('admin_dashboard'))
...
def ai_assist():
    ...
    prompt = f"Write detailed UPSC study notes on {topic}. Use HTML tags like <h3>, <ul>, <li>. Bilingual Hindi/English."
